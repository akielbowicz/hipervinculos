...
2. Share → Telegram → Your bookmark bot
3. Bot responds with preview + "✅ Saved"
4. Continue browsing
...
2. **Duplicate check** (if found):
   ```
   ⚠️ Already bookmarked
   
   📄 Article Title
   🔗 https://example.com/article
   📅 Saved on Jan 10, 2025
   🆔 abc-123
   
   Save anyway? This will create a duplicate.
   [✅ Save Anyway] [👁️ View Existing]
   ```
...
3. **Preview card** (new bookmark):
   ...
   [✅ Save] [✏️ Edit Before Save] [❌ Cancel]
   ```
...
4. **After save confirmation**:
   ...
   [🏷️ Add Tags] [⭐ Favorite] [🔒 Make Private]
   ```
...
[✏️ Edit Title] [✏️ Edit Description] [🏷️ Edit Tags]
[📝 Add Note] [❌ Cancel]
```
...
```
🔍 Found 3 results for "react hooks"
...
- Buttons update after action (✅ becomes Saved!)
...
**Read Operations:**
```javascript
GET /repos/{owner}/{repo}/contents/data/bookmarks.json
- Parse JSON
...
**Write Operations:**
```javascript
PUT /repos/{owner}/{repo}/contents/data/bookmarks.json
...
1. Normalize URL
2. Generate url_hash
3. Search bookmarks.json for matching hash
4. If found:
...
**Keys:**
```
retry:{uuid} → {bookmark_object, attempts, next_retry}
cache:{url_hash} → {metadata, cached_at}
ratelimit:{user_id}:{minute} → {count}
token:{hash} → {created_at, last_used}
```
...
│  🔖 My Bookmarks              [Search...    🔍] │
├─────────────────────────────────────────────────┤
│ [All] [Unread] [Favorites] [Private]           │
│ Type: [All ▾] Tags: [All ▾] Sort: [Newest ▾]   │
├─────────────────────────────────────────────────┤
│ ┌─────────┐ ┌─────────┐ ┌─────────┐           │
│ │ [Image] │ │ [Image] │ │ [Image] │  3-col    │
...
<article class="bookmark-card" data-id="{id}">
  ...
  <div class="actions">
    <button class="favorite" aria-label="Favorite">⭐</button>
...
**Special Views:**
- Favorites (⭐ icon, filtered)
- Private (🔒 icon, needs auth)
- Unread (badge count)
...
**Search Syntax:**
```
Query: "react hooks"          → Phrase search
Query: react AND hooks        → Both terms required
Query: react OR vue           → Either term
Query: react -angular         → Exclude term
Query: tag:javascript         → Filter by tag
Query: type:article           → Filter by type
Query: site:example.com       → Filter by domain
```
...
• Removing filters
• Searching all bookmarks (not just unread)
```
...
**Caching:**
- Service Worker caches bookmarks.json (1 hour)
...
**Trigger:**
- Push to main with changes in `data/bookmarks.json`
...
**Script Logic:**
```javascript
const bookmarks = JSON.parse(fs.readFileSync('data/bookmarks.json'));
...
**Actions:**
- Create timestamped backup of bookmarks.json
...
```javascript
Cache Strategy:
- bookmarks.json: Network first, fallback to cache (1 hour)
...
- Stored in same bookmarks.json (but filtered)
...
- bookmarks.json size approaching 10MB
...
Output:
✅ 245 bookmarks imported
⚠️ 12 duplicates skipped
❌ 3 failed (URLs inaccessible)
```
...
# JSON (raw data)
node scripts/export.js --format json --output bookmarks.json
...
```bash
# Restore from backup
git checkout origin/main data/bookmarks.json
git push origin main
```
...
### Week 3: Site & Search
- Build script (bookmarks → HTML pages)
- Integrate Pagefind
- Implement filters
- Browse mode UI
- Mobile responsive
